...
/* LITTLE MODIFICATION ON BELOW CODE: to solve "Find all Anagrams in a string so both similar problems " */
...
// if s2 contains permutations of s1
    int checkInclusion(string s1, string s2) {
        ...
        vector<string> ans; // to store the ans
        int cnt = 0;
        while(r < n2) {
            // INCLUDE 
            mp2[s2[r]]++;
            ...
            if(winLen == n1) {
                if(winCnt == n1) {
                    cnt++;
                    ans.push_back(s2.substr(l, r-l+1));
                }
                // EXCLUDE
        ...
        /* Print out all permutations of s1 in s2 and return the cnt */
        for(int i = 0; i < cnt; i++) {
            cout << ans[i] << " ";
        }
        cout << endl;
